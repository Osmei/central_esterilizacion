...
class UsuarioRepository extends AbstractRepository {
    ...
    public function getByUsuario(string $usuario): ?Usuario
    {
        $sql = "SELECT *               
                FROM usuarios u
                WHERE u.es_activo = '1' 
    ...
    private function createFromResultset($result, array $fields, $db) {
        ...
        $item->setApellido($result->apellido);
        $item->setUsuario($result->usuario);
        $item->setLaboratorioId($result->laboratorioId);
        $item->setClave($result->clave);
        $item->setClaveActivacionCodigo($result->clave_activacion_codigo);
        $item->setClaveActivacionExpiracion($result->clave_activacion_expiracion);
        
        $item->setActivo($result->es_activo);
        
        if (in_array('*', $fields) || in_array('perfil', $fields))
            $item->setPerfil((new PerfilesRepository($db))->get($result->perfil));
